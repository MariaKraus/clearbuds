...
class Solver(object):
    ...
    def __init__(self, data, model, optimizer, args):
        ...
        self.mrstftloss = MultiResolutionSTFTLoss(factor_sc=0.5,
                                                  factor_mag=0.5).to(torch.device("cuda"))
        self.fftloss = FourierTransform(sample_rate=15625)
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, (data) in enumerate(data_loader):
            ...
            mixture_lengths += PADDING_AMOUNT

            estimate_source = self.model(padded_mixture)

            t_dim = int(estimate_source.shape[-1])
            if LOOKAHEAD != 0:
                gt = padded_source[:, 0, :, -(t_dim + LOOKAHEAD):-LOOKAHEAD]
            else:
                gt = padded_source[:, 0, :, -t_dim:]
            l1_loss = F.l1_loss(gt, estimate_source)
            # loss = l1_loss
            sc_loss1, mag_loss1 = self.mrstftloss(gt[:, 0], estimate_source[:, 0])
            # sc_loss2, mag_loss2 = self.mrstftloss(gt[:, 1], estimate_source[:, 1])
            loss = 5 * l1_loss + 0.2 * sc_loss1 + 0.2 * mag_loss1
            # loss = F.mse_loss(gt, estimate_source)
            # loss = self.fftloss.loss(estimate_source[:, 0], gt[:, 0])
            # loss, max_snr, estimate_source, reorder_estimate_source = \
            #      cal_loss(gt, estimate_source, mixture_lengths)
            if not cross_valid:
            ...
            if i % self.print_freq == 0:
                print('Epoch {0} | Iter {1} | Average Loss {2:.3f} | '
                      'Current Loss {3:.6f} | L1 Loss: {4: .5f} | {5:.1f} ms/batch'.format(
                          epoch + 1, i + 1, total_loss / (i + 1),
                          loss.item(), l1_loss, 1000 * (time.time() - start) / (i + 1)),
                      flush=True)
